      ...
      <aside className="hidden w-64 flex-col border-r border-gray-200 bg-white lg:flex overflow-hidden">
        ...
        <nav className="flex-1 space-y-1 p-3">
          ...
          <Link
            ...
            >
              <span className={isActive(item.href, item.exact) ? "" : "text-gray-400"}>
                {item.icon}
              </span>
              {item.label}
            </Link>
        ...
        <div className="border-t border-gray-200 p-3 space-y-1">
          <Link
            href="/"
            className="flex items-center gap-3 rounded-xl bg-[linear-gradient(135deg,#0d2d62,#2e7ef7)] px-3 py-2.5 text-sm font-semibold text-white transition hover:opacity-90"
          >
            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.75}>
